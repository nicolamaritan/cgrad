...
// Example dataset build for classification
void build_example_dataset(tensor *x, tensor *y_target);
// Assigns a class label and generates features clustered by class
void generate_class_example(double *x_row, int class_label, size_t dim);
...
int main()
{
    ...
    const size_t out_dim = NUM_CLASSES;
...
// Generate features for a given class label
void generate_class_example(double *x_row, int class_label, size_t dim)
{
    double center = 10.0 * class_label; // cluster centers: 0, 10, 20
    for (size_t j = 0; j < dim; j++) {
        x_row[j] = sample_uniform(center - 2.0, center + 2.0);
    }
}

// Build dataset: features + integer class labels
void build_example_dataset(tensor *x, tensor *y_target)
{
    for (size_t i = 0; i < x->shape[0]; i++) 
    {
        int class_label = rand() % NUM_CLASSES;
        double x_row[x->shape[1]];
        generate_class_example(x_row, class_label, x->shape[1]);
        for (size_t j = 0; j < x->shape[1]; j++) 
        {
            tensor2d_set_unchecked(x, i, j, x_row[j]);
        }
        tensor2d_set_unchecked(y_target, i, 0, (double)class_label);
    }
}